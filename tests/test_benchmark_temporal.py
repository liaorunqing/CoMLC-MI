...
from src import benchmark_temporal
...
def test_temporal_shard_writes_checkpoint_without_assembly(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(benchmark_temporal.pd, "read_csv", lambda _: data)
    monkeypatch.setattr(benchmark_temporal, "prepare_outcomes", lambda _: pd.DataFrame(outcomes))
    monkeypatch.setattr(benchmark_temporal, "repeated_outer_folds", lambda *args: [fold])
    monkeypatch.setattr(
        benchmark_temporal,
        "inner_folds",
        lambda *args: iter([(np.array([0, 1]), np.array([2, 3]))]),
    )
    monkeypatch.setattr(benchmark_temporal, "_processor", lambda *args: _IdentityProcessor())
    monkeypatch.setattr(benchmark_temporal, "neural_best_epoch", lambda *args, **kwargs: 2)
    monkeypatch.setattr(
        benchmark_temporal,
        "fit_predict_model",
    ...
    metrics, labels = benchmark_temporal.run_temporal(
        config,
        tmp_path,
    ...
    checkpoint = tmp_path / "temporal_admission_safe_v1_rep1_fold1.npz"
