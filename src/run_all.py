...
if __name__ == "__main__":
    raise SystemExit(
        "src.run_all is a retired fixed-split workflow and is not part of the "
        "IEEE Access R1 analysis. Use: python -m src.run_revision --config "
        "configs/access_2026_35668_r1.json"
    )
...
from statistical_utils import benjamini_hochberg, delong_roc_test, permutation_auc_test
...
# --- 4a. LP-RF (without synthetic oversampling) ---
print("  [a] LP-RF...")
...
# --- 4c. TabPFN-3-BR (per-label, top-80 RF-selected features) ---
print("  [c] TabPFN-3-BR (12 labels)...")
...
# ═══════════════════════════════════════════════════════════════════════════════
# 7. PATIENT-LEVEL BOOTSTRAP (valid multi-label resamples)
# ═══════════════════════════════════════════════════════════════════════════════
print("\n[5/6] Patient-level bootstrap robustness (500 valid resamples)...")
N_BOOT = 500
n_test = len(y_test)
...
while len(boot_results[next(iter(boot_results))]['macro_auc']) < N_BOOT:
    bootstrap_attempts += 1
    idx = boot_rng.choice(n_test, n_test, replace=True)
    y_boot = y_test[idx]
    # A macro-AUC resample is defined only when every label contains both
    # classes. Reject invalid draws before evaluating any model so all models
    # use exactly the same 500 patient resamples.
    if any(np.unique(y_boot[:, label_index]).size < 2 for label_index in range(N_LABELS)):
        continue
    ...
    completed = len(boot_results[next(iter(boot_results))]['macro_auc'])
    if completed % 100 == 0:
        print(f"  Bootstrap {completed}/{N_BOOT} valid resamples...")
print(f"  Accepted {N_BOOT} of {bootstrap_attempts} patient-level resamples.")
...
# Ensemble vs TabPFN
ens_vs_tpf = run_label_tests(y_test, ens2_wt, tpf_test, 'TabPFN')
# Ensemble vs LP-RF
ens_vs_lp = run_label_tests(y_test, ens2_wt, lp_test, 'LP-RF')
...
for results_list in [ens_vs_tpf, ens_vs_lp]:
    ...
    if len(p_vals) > 0:
        reject, p_fdr = benjamini_hochberg(p_vals)
        j = 0
...
print(f"\n{'='*70}")
print("PUBLICATION-READY TABLE: Ensemble 2-model (validation-AUC weighted) vs TabPFN-3-BR")
print(f"{'='*70}")
...
full_results = {
    'metadata': {
        ...
        'note': 'All models trained on original data without MLSMOTE. '
                ...
                'Fixed seeds and the software environment are recorded for reproducibility.',
    },
    'test_metrics': {
...
# --- Calibration results for the two strongest single models and the primary
# validation-weighted ensemble. Reliability-bin data are retained for plotting.
calibration_rows = []
calibration_bins = {}
for calibration_name, calibration_predictions in [
        ('lp_rf', lp_test), ('tabpfn', tpf_test),
        ('catboost', cb_test), ('ens2_wt', ens2_wt)]:
    rows, bins = evaluate_calibration(y_test, calibration_predictions, calibration_name)
    calibration_rows.extend(rows)
    calibration_bins.update(bins)
pd.DataFrame(calibration_rows).to_csv(
    os.path.join(OUTPUT_DIR, 'CALIBRATION_RESULTS.csv'), index=False
)
with open(os.path.join(OUTPUT_DIR, 'CALIBRATION_RELIABILITY_BINS.json'), 'w') as f:
    json.dump(calibration_bins, f, indent=2)
...
best_single_macro = max(all_metrics['LP-RF']['macro_auc'],
                        all_metrics['TabPFN-3-BR']['macro_auc'])
...
print("  Fixed seeds and the software environment have been recorded.")
