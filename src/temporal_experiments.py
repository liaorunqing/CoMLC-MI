...
================================================================================
Trains and evaluates one prespecified no-graph neural model at four horizons:
...
OUTPUT_DIR = os.path.join(PROJECT_DIR, 'output', 'temporal_corrected')
...
ALL_NON_INTERVAL_FEATURES = [
    ...
    # Time to hospital
    'TIME_B_S',
    # Medications recorded as non-interval variables in the source dictionary
    'NA_KB','NOT_NA_KB','LID_KB','NITR_S',
    'LID_S_n','B_BLOK_S_n','ANT_CA_S_n','GEPAR_S_n',
    'ASP_S_n','TIKL_S_n','TRENT_S_n',
]

# The source data dictionary assigns the *_1, *_2 and *_3 count variables to
# successive observation intervals. Future intervals are excluded at each
# prediction horizon to prevent temporal leakage.
T0_FEATURES = ALL_NON_INTERVAL_FEATURES
T1_FEATURES = T0_FEATURES + ['R_AB_1_n', 'NA_R_1_n', 'NOT_NA_1_n']
T2_FEATURES = T1_FEATURES + ['R_AB_2_n', 'NA_R_2_n', 'NOT_NA_2_n']
T3_FEATURES = T2_FEATURES + ['R_AB_3_n', 'NA_R_3_n', 'NOT_NA_3_n']
...
print(f"Train: {len(X_train_full)}, Validation: {len(X_val)}, Test: {len(X_test)}")

for name, feats in HORIZONS.items():
    available = [f for f in feats if f in X_train_full.columns]
    print(f"  {name}: {len(available)} features available")

# ── Model Definition ─────────────────────────────────────────────────────
class BestCoMLCMI(nn.Module):
    """Prespecified no-graph neural baseline with a dot-product head."""
    ...
    def forward(self, x):
        h = self.encoder(x)
        return torch.matmul(h, self.label_embs.T)


def train_on_horizon(name, feature_list, X_tr, y_tr, X_va, y_va, X_te, y_te):
    """Train the same neural baseline on a specific temporal feature subset."""
    ...
    X_va_h = X_va[available].values.astype(np.float32)
    X_te_h = X_te[available].values.astype(np.float32)

    prevalence = y_tr.mean(axis=0)
    pos_weights = (1.0 - prevalence) / (prevalence + 1e-6)

    X_tr_t = torch.FloatTensor(X_tr_h)
    y_tr_t = torch.FloatTensor(y_tr)
    X_va_t = torch.FloatTensor(X_va_h)
    ...
    train_loader = DataLoader(train_ds, batch_size=64, shuffle=True, generator=loader_generator)

    model = BestCoMLCMI(n_feats, N_L, d=64, dropout=0.3).to(DEVICE)
    optimizer = optim.AdamW(model.parameters(), lr=1e-3, weight_decay=1e-4)
    scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=20, T_mult=2)
    loss_function = nn.BCEWithLogitsLoss(pos_weight=torch.FloatTensor(pos_weights).to(DEVICE))
    ...
    model.eval()
    with torch.no_grad():
        test_preds = torch.sigmoid(model(X_te_t.to(DEVICE))).cpu().numpy()
...
horizon_results = {}
for name, feats in HORIZONS.items():
    print(f"\n  Training {name} ({len([f for f in feats if f in X_train_full.columns])} features)...")
    result = train_on_horizon(
        name, feats, X_train_full, y_train, X_val, y_val, X_test, y_test
    )
