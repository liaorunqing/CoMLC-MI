"""Acceptance gates for the CoMLC-MI benchmark and submission artifacts.

Run the analysis gates as soon as the locked experiments finish::

    python -m src.benchmark_qa --level analysis

Use ``--level submission`` only after the clean manuscript, highlighted PDF,
response letter, integrated appendices, and release archive have been generated.
"""
...
from .benchmark_features import feature_sets
...
CONFIG_PATH = ROOT / "configs" / "comlc_mi_benchmark.json"
...
def submission_artifact_gates() -> dict[str, object]:
    submission = RESULTS.parent / "final_delivery"
    required = {
