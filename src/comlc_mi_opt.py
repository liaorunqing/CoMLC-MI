...
Tests multiple configurations:
  ...
  - Loss: BCE (weighted), prevalence-weighted ASL (various gamma_minus, m)
...
OUTPUT_DIR = os.path.join(PROJECT_DIR, 'output', 'comlc_mi')
RANDOM_SEED = 42
...
def prevalence_weighted_asl_loss(preds, targets, gamma_minus=3, m=0.05):
    """Prevalence-weighted adaptation of Asymmetric Loss (Ridnik et al.).

    Positive log loss is multiplied by the training-set negative-to-positive
    ratio; the margin-shifted, focused negative term is divided by that ratio.
    This is a study-specific adaptation, not the original ASL formula.
    """
    w = torch.FloatTensor(pos_weights).to(preds.device)
    eps = 1e-8
...
for adj_name, (ei, ew) in adj_configs.items():
    for model_cls, model_name in [(DotProductModel, 'dot'), (ConcatModel, 'concat')]:
        for loss_fn, loss_name in [
            ...
            (lambda p, t: prevalence_weighted_asl_loss(p, t, gamma_minus=2, m=0.05), 'W_ASL_g2'),
            (lambda p, t: prevalence_weighted_asl_loss(p, t, gamma_minus=3, m=0.05), 'W_ASL_g3'),
        ]:
            experiments.append({
