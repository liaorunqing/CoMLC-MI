...
from .benchmark_features import FoldPreprocessor, feature_sets, prepare_outcomes
from .benchmark_metrics import aggregate_metrics, per_label_metrics
from .benchmark_models import fit_predict_model, neural_best_epoch
    ...
    for horizon_index, (horizon, contract) in enumerate(HORIZONS):
        ...
        for fold_ordinal, fold in enumerate(folds):
            if requested_folds is not None and fold_ordinal not in requested_folds:
                continue
            checkpoint = output_dir / f"temporal_{contract}_rep{fold.repeat + 1}_fold{fold.fold + 1}.npz"
