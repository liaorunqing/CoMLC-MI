...
Implements the tiered missing-data strategy (Section 4.3):
  ...
  2. Retain EMS BP (63% missing) with a missingness indicator
  3. One iterative Random-Forest completed-data imputation (10 iterations)
...
# Lab features assigned to iterative RF imputation (15-25% missing)
LAB_FEATURES = ['GIPO_K', 'K_BLOOD', 'GIPER_NA', 'NA_BLOOD',
                'ALT_BLOOD', 'AST_BLOOD', 'L_BLOOD', 'ROE']

# Moderate missing (10-15%): also assigned to iterative RF imputation
...
if not np.isin(y.to_numpy(), (0, 1)).all():
    raise ValueError("All outcomes must be binary after LET_IS conversion.")
...
# Step 2b: Split before imputation to avoid data leakage
...
# Step 2c: iterative RF imputation fitted on training data
print("\n2c. Running iterative RF imputation (10 max_iter)...")
iterative_rf_features = LAB_FEATURES + MODERATE_MISSING + HIGH_MISSING_FEATURES
iterative_rf_features = [f for f in iterative_rf_features if f in X.columns]

# Fit the iterative imputer on training data
iterative_rf_imputer = IterativeImputer(
    estimator=RandomForestRegressor(n_estimators=50, random_state=RANDOM_SEED),
    max_iter=10,
    random_state=RANDOM_SEED,
    initial_strategy='median'
)
X_train_iterative_rf = X_train_raw.copy()
X_val_iterative_rf = X_val_raw.copy()
X_test_iterative_rf = X_test_raw.copy()

# Apply one iterative Random-Forest completed-data imputation to selected features
if iterative_rf_features:
    X_train_iterative_rf[iterative_rf_features] = iterative_rf_imputer.fit_transform(
        X_train_raw[iterative_rf_features]
    )
    X_val_iterative_rf[iterative_rf_features] = iterative_rf_imputer.transform(
        X_val_raw[iterative_rf_features]
    )
    X_test_iterative_rf[iterative_rf_features] = iterative_rf_imputer.transform(
        X_test_raw[iterative_rf_features]
    )
    print(
        f"  Iterative RF imputation applied to {len(iterative_rf_features)} "
        f"features: {iterative_rf_features}"
    )

# Step 2d: Simple imputation for remaining features
print("\n2d. Applying median/mode imputation for remaining features...")
remaining_features = [f for f in X.columns if f not in iterative_rf_features]
if remaining_features:
    # Use median for numeric, mode for binary
    simple_imputer = SimpleImputer(strategy='median')
    X_train_imputed = X_train_iterative_rf.copy()
    X_val_imputed = X_val_iterative_rf.copy()
    X_test_imputed = X_test_iterative_rf.copy()
    ...
    X_train_imputed = X_train_iterative_rf
    X_val_imputed = X_val_iterative_rf
    X_test_imputed = X_test_iterative_rf
...
with open(os.path.join(OUTPUT_DIR, 'iterative_rf_imputer.pkl'), 'wb') as f:
    pickle.dump(iterative_rf_imputer, f)
...
# Save label-space diagnostics calculated from binary outcomes.  Training-set
# metrics are the quantities used for model selection; full-cohort metrics are
# included only for descriptive reporting.
label_metrics = {
    'definition': (
    ...
    'full_cohort': label_space_metrics(y.to_numpy()),
    'train': label_space_metrics(y_train.to_numpy()),
    'validation': label_space_metrics(y_val.to_numpy()),
    'test': label_space_metrics(y_test.to_numpy()),
}
with open(os.path.join(OUTPUT_DIR, 'label_space_metrics.json'), 'w') as f:
